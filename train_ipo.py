...
def create_ipo_clients(
    ...
    if resume_info:
        # Resuming interrupted IPO training - load weights + optimizer state
        ...
        logger.info(f"Resumed IPO training from {resume_info.state_path}")
    elif config.load_checkpoint_path:
        # Starting fresh IPO from checkpoint - load weights only (fresh optimizer)
        checkpoint_utils.check_renderer_name_for_checkpoint(
...
def compute_ipo_loss(
    ...
    # Compute IPO loss
    losses = (chosen_log_ratio - rejected_log_ratio - 1 / (2 * ipo_beta)) ** 2
    loss = losses.mean()

    # Compute metrics
    accuracy = (chosen_log_ratio > rejected_log_ratio).float().mean().item()
    chosen_rewards = ipo_beta * chosen_log_ratio
    rejected_rewards = ipo_beta * rejected_log_ratio
    ...
    with trace.trace_iteration(step=step) as window:
        ...
        def ipo_loss_fn(
            ...
            return compute_ipo_loss(
                ...
                ipo_beta=config.ipo_beta,
            )

        with trace.scope_span_sync("step"):
            # Do forward-backward with custom IPO loss
            backward_result = training_client.forward_backward_custom(data, ipo_loss_fn).result()
            ipo_metrics = backward_result.metrics
...
def main(config: Config):
    ...
    training_client, reference_client = create_ipo_clients(config, resume_info, user_metadata)
    service_client = tinker.ServiceClient(base_url=config.base_url)
